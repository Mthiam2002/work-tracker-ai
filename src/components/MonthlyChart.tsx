  ...
  return (
    <section className="mt-4 rounded-3xl border border-zinc-200/80 bg-white p-6 sm:p-8 dark:border-white/10 dark:bg-zinc-950">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-xs text-zinc-500 dark:text-zinc-400">Salaire estimé / mois</span>
      </div>
          ...
          {points.map((p, i) => {
            ...
            return (
              <div key={p.key} className="flex flex-1 flex-col items-center gap-2" title={`${p.label} : ${hoursLabel(p.hours)} · ${eur.format(p.pay)}`}>
                <span className="text-[11px] font-medium text-zinc-500 sm:text-xs dark:text-zinc-400">
                  {p.pay > 0 ? eur.format(p.pay) : "—"}
                </span>
                <div
                  style={{ height }}
                  className={`w-full rounded-t-xl transition ${
                    isCurrent ? "bg-blue-600" : "bg-blue-600/35 dark:bg-blue-500/40"
                  }`}
                />
                <span className={`text-[11px] capitalize sm:text-xs ${isCurrent ? "font-semibold" : "text-zinc-500 dark:text-zinc-400"}`}>
                  {p.label}
                </span>
              </div>
            );
