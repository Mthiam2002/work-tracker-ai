...
import { deletePayslip } from "@/app/(protected)/payslips/payslips-actions";

export function DeletePayslipButton({ id }: { id: string }) {
  ...
  return (
    <span className="flex items-center gap-1.5">
      <button
        type="button"
        disabled={isPending}
        onClick={() => startTransition(async () => { await deletePayslip(id); })}
